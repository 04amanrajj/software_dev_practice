...
// Old method
{
  ...
  //       let posts = fetchPosts(user.userId);
  //       posts.then((res) => {
  //         for (let post of res) {
      ...
      for (let user of users) {
        ...
        for (let post of posts) {
          ...
          for (let comment of comments) {
            console.log("Comments", comment.body);
          }
        }
      }
    } catch (err) {
...
{
  fetchUsersCB((rej, users) => {
    if (rej) console.log(rej.message);
    else {
      for (let user of users) {
        console.log("ID", user.userId, "Name", user.name);
        fetchPostsCB(user.userId, (rejectPost, posts) => {
          if (rejectPost) console.log(rejectPost.message);
          else {
            for (let post of posts) {
              console.log("Post Title", post.title);
              fetchCommentsCB(post.postId, (rejectComment, comments) => {
                if (rejectComment) console.log(rejectComment.message);
                else {
                  for (let comment of comments) {
                    console.log("Comments", comment.body);
                  }
